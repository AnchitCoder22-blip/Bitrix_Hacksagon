...
export default function AnalyticsPage() {
  const { queue, served } = useSystem();
  const { peakHours, waitTrends, doctorPerformance } = ANALYTICS_DATA;

  const totalPatientsToday = queue.length + served.length;
  // Combine real today's patients with historical data
  const patientFlow = {
    total: 1247 + served.length,
