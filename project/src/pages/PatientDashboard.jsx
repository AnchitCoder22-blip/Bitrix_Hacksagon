...
export default function PatientDashboard() {
  const { onNav, tokens, user, queue, getQueueHealth, estimateWaitTime, getPosition, tracking, trackingWarning } = useSystem();
  const activeTokens = tokens.filter(t => t.status !== 'completed' && t.status !== 'served');
    ...
    <div>
      ...
      {trackingWarning && (
        <motion.div
          ...
          style={{
             ...
             border: `1px solid ${trackingWarning.type === 'warning' ? '#ef444430' : '#22c55e30'}`,
             color: trackingWarning.type === 'warning' ? '#ef4444' : '#22c55e',
             padding: '12px 18px', borderRadius: 12, fontSize: '0.88rem', fontWeight: 600,
             display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer',
          }}
          onClick={() => onNav('tracking')}
        >
          {trackingWarning.type === 'warning' ? '⚠️' : '✅'} 
          <span style={{ flex: 1 }}>{trackingWarning.text}! Click to view live tracking details. ETA: ~{tracking?.eta}m</span>
          <Icon name="navigation" size={16} color={trackingWarning.type === 'warning' ? '#ef4444' : '#22c55e'} />
        </motion.div>
      )}
      ...
      >
        <motion.div
          ...
          style={{ width: 10, height: 10, borderRadius: '50%', background: health.color }}
        />
        <span style={{ fontSize: '0.88rem', fontWeight: 600, color: health.color }}>
          Queue Status: {health.label}
        </span>
        <span style={{ fontSize: '0.82rem', color: '#64748b', marginLeft: 'auto' }}>
          {queue.length} in queue • {health.totalServed} served today
        </span>
      </motion.div>

      {/* Banner */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} style={{ marginBottom: 28 }}>
        <BannerCarousel />
      </motion.div>
        ...
        {activeTokens.length === 0 ? (
          <div style={{
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              onClick={() => onNav('doctors')} className="btn-primary" style={{ padding: '10px 24px' }}>
              Find a Doctor
            </motion.button>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {activeTokens.map((token, i) => {
              const pos = getPosition(token.id);
              const wait = pos !== null ? estimateWaitTime(pos) : null;
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {wait !== null && (
                      <div style={{ fontSize: '0.78rem', color: '#f59e0b', fontWeight: 600, marginTop: 2 }}>
                        ~{wait}m estimated wait • Position {pos + 1}
                      </div>
                    )}
                  </div>
                  <span className={`badge badge-${token.status === 'arrived' ? 'success' : token.isEmergency ? 'error' : token.status === 'ongoing' ? 'info' : 'warning'}`}>
                    {token.status === 'arrived' ? 'Arrived' : token.isEmergency ? 'Emergency' : token.status === 'ongoing' ? 'In Progress' : 'Waiting'}
                  </span>
                </motion.div>
              );
            })}
