import { motion } from 'framer-motion';
import { useSystem } from '../context/SystemContext';
import Icon from '../shared/Icon';
import Avatar from '../shared/Avatar';

export default function AdminDashboard() {
  const {
    doctors, queue, secondaryQueue, served, currentPatient,
    getQueueHealth, moveUp, moveDown, markEmergency, removePatient,
    reAddFromSecondary, handleNoShow, toggleDoctorAvailability,
    addNotification, estimateWaitTime, addToast
  } = useSystem();

  const [broadcastMsg, setBroadcastMsg] = React.useState('');
  const health = getQueueHealth();

  const sendBroadcast = () => {
    if (!broadcastMsg.trim()) return;
    addToast?.(`Broadcast sent: "${broadcastMsg}"`, 'info');
    addNotification(`📢 ${broadcastMsg}`, 'info');
    setBroadcastMsg('');
  };

  const activeDoctors = doctors.filter(d => d.available).length;
  const avgWaitPerDoctor = Math.round((queue.length * 15) / Math.max(activeDoctors, 1));

  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: 4 }}>Admin Dashboard</h2>
        <p style={{ color: '#64748b' }}>System control center — Real-time queue management</p>
      </motion.div>

      {/* Queue Health Banner */}
      <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        style={{
          background: `linear-gradient(135deg, ${health.color}15, ${health.color}08)`,
          borderRadius: 16, padding: '16px 20px', marginBottom: 20,
          border: `1px solid ${health.color}30`,
          display: 'flex', alignItems: 'center', gap: 14,
        }}
      >
        <motion.div
          animate={{ scale: [1, 1.2, 1] }}
          transition={{ repeat: Infinity, duration: 2 }}
          style={{
            width: 12, height: 12, borderRadius: '50%',
            background: health.color,
            boxShadow: `0 0 12px ${health.color}60`,
          }}
        />
        <div style={{ flex: 1 }}>
          <span style={{ fontWeight: 700, color: health.color }}>{health.label}</span>
          <span style={{ color: '#64748b', fontSize: '0.85rem', marginLeft: 12 }}>
            {health.waitingCount} waiting • {health.arrivedCount} arrived • {health.noShowCount} no-shows
          </span>
        </div>
        <div style={{ fontSize: '0.82rem', color: '#64748b' }}>
          Avg Wait: <strong style={{ color: health.color }}>{avgWaitPerDoctor}m</strong>
        </div>
      </motion.div>

      {/* Stats */}
      <div style={{
        display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))',
        gap: 16, marginBottom: 24,
      }}>
        {[
          { label: 'Total Patients', value: queue.length + served.length, icon: 'users', color: '#2563EB', gradient: 'linear-gradient(135deg, #dbeafe, #eff6ff)' },
          { label: 'In Queue', value: queue.length, icon: 'queue', color: '#14B8A6', gradient: 'linear-gradient(135deg, #ccfbf1, #f0fdfa)' },
          { label: 'Served Today', value: served.length, icon: 'check', color: '#22c55e', gradient: 'linear-gradient(135deg, #dcfce7, #f0fdf4)' },
          { label: 'Active Doctors', value: `${activeDoctors}/${doctors.length}`, icon: 'doctor', color: '#8b5cf6', gradient: 'linear-gradient(135deg, #ede9fe, #f5f3ff)' },
          { label: 'No-Shows', value: health.noShowCount, icon: 'alert', color: '#ef4444', gradient: 'linear-gradient(135deg, #fecaca20, #fef2f220)' },
          { label: 'Avg Wait', value: `${avgWaitPerDoctor}m`, icon: 'clock', color: '#f59e0b', gradient: 'linear-gradient(135deg, #fef3c7, #fffbeb)' },
        ].map((stat, i) => (
          <motion.div
            key={i}
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            transition={{ delay: i * 0.05 }}
            whileHover={{ y: -4, boxShadow: '0 8px 30px rgba(0,0,0,0.08)' }}
            style={{
              background: stat.gradient, borderRadius: 18, padding: 20,
              border: `1px solid ${stat.color}20`, cursor: 'default',
              transition: 'all 0.3s',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <Icon name={stat.icon} size={18} color={stat.color} />
              <span style={{ fontSize: '0.82rem', color: '#64748b', fontWeight: 500 }}>{stat.label}</span>
            </div>
            <div style={{ fontSize: '1.5rem', fontWeight: 800, color: stat.color }}>{stat.value}</div>
          </motion.div>
        ))}
      </div>

      {/* Currently Serving */}
      {currentPatient && (
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          style={{
            background: 'linear-gradient(135deg, #2563EB, #14B8A6)',
            borderRadius: 20, padding: 24, color: 'white', marginBottom: 24,
          }}
        >
          <p style={{ opacity: 0.8, fontSize: '0.82rem', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>NOW SERVING</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <span style={{ fontSize: '2rem', fontWeight: 900 }}>#{currentPatient.tokenNumber}</span>
            <div>
              <div style={{ fontWeight: 700, fontSize: '1.1rem' }}>{currentPatient.name}</div>
              <div style={{ opacity: 0.8, fontSize: '0.85rem' }}>
                {currentPatient.doctorName} • {currentPatient.isEmergency ? '🚨 Emergency' : 'Regular'}
              </div>
            </div>
          </div>
        </motion.div>
      )}

      {/* Doctor Management */}
      <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        style={{
          background: 'white', borderRadius: 20, padding: 24,
          boxShadow: '0 4px 20px rgba(0,0,0,0.05)', marginBottom: 24,
        }}
      >
        <h3 style={{ fontWeight: 700, marginBottom: 16 }}>Doctor Management</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {doctors.map((doc) => (
            <motion.div key={doc.id}
              whileHover={{ backgroundColor: '#f1f5f9' }}
              style={{
                display: 'flex', alignItems: 'center', gap: 14,
                padding: '12px 14px', borderRadius: 14,
                background: '#f8fafc', transition: 'all 0.2s',
              }}
            >
              <Avatar emoji={doc.avatar} size={40} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{doc.name}</div>
                <div style={{ fontSize: '0.78rem', color: '#64748b' }}>
                  {doc.specialty} • {doc.patients} patients • {doc.avgTime}m avg
                </div>
              </div>
              <motion.button
                whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                onClick={() => {
                  toggleDoctorAvailability(doc.id);
                  addToast?.(`${doc.name} ${doc.available ? 'deactivated' : 'activated'}`, doc.available ? 'warning' : 'success');
                }}
                style={{
                  padding: '6px 14px', borderRadius: 8, fontSize: '0.78rem', fontWeight: 600,
                  background: doc.available ? '#dcfce7' : '#fecaca',
                  color: doc.available ? '#15803d' : '#dc2626',
                  border: 'none', cursor: 'pointer',
                }}
              >
                {doc.available ? 'Active' : 'Inactive'}
              </motion.button>
            </motion.div>
          ))}
        </div>
      </motion.div>

      {/* Queue Control */}
      <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        style={{
          background: 'white', borderRadius: 20, padding: 24,
          boxShadow: '0 4px 20px rgba(0,0,0,0.05)', marginBottom: 24,
        }}
      >
        <h3 style={{ fontWeight: 700, marginBottom: 16 }}>Queue Control ({queue.length} patients)</h3>
        {queue.length === 0 ? (
          <div style={{ color: '#94a3b8', textAlign: 'center', padding: 32 }}>
            <span style={{ fontSize: 48, display: 'block', marginBottom: 12 }}>📋</span>
            <p>No patients in queue</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {queue.map((p, i) => (
              <motion.div key={p.id}
                initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.03 }}
                whileHover={{ backgroundColor: p.isEmergency ? '#fef2f2' : '#f1f5f9' }}
                style={{
                  display: 'flex', alignItems: 'center', gap: 12,
                  padding: '10px 14px', borderRadius: 12,
                  background: p.isEmergency ? '#fef2f2' : '#f8fafc',
                  borderLeft: `3px solid ${p.isEmergency ? '#ef4444' : p.status === 'arrived' ? '#22c55e' : '#e2e8f0'}`,
                  transition: 'all 0.2s',
                }}
              >
                <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#2563EB', minWidth: 50 }}>
                  #{p.tokenNumber}
                </span>
                <div style={{ flex: 1 }}>
                  <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>{p.name}</span>
                  <div style={{ fontSize: '0.72rem', color: '#94a3b8' }}>
                    {p.status === 'arrived' ? '✓ Arrived' : '⏳ Waiting'}
                    {p.isEmergency ? ' • 🚨 Emergency' : ''}
                    {p.isVIP ? ' • ⭐ VIP' : ''}
                    {' • ~'}{estimateWaitTime(i)}m wait
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 4 }}>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => moveUp(p.id)}
                    style={{ width: 30, height: 30, borderRadius: 8, background: '#e0e7ff', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                    <Icon name="arrowUp" size={14} color="#2563EB" />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => moveDown(p.id)}
                    style={{ width: 30, height: 30, borderRadius: 8, background: '#e0e7ff', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                    <Icon name="arrowDown" size={14} color="#2563EB" />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                    onClick={() => { markEmergency(p.id); addToast?.(`${p.name} marked as emergency`, 'warning'); }}
                    style={{ width: 30, height: 30, borderRadius: 8, background: '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                    <Icon name="alert" size={14} color="#f59e0b" />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                    onClick={() => { handleNoShow(p.id); addToast?.(`${p.name} marked as no-show`, 'error'); }}
                    style={{ width: 30, height: 30, borderRadius: 8, background: '#fecaca', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                    <Icon name="close" size={14} color="#dc2626" />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                    onClick={() => { removePatient(p.id); addToast?.(`${p.name} removed from queue`, 'error'); }}
                    style={{ width: 30, height: 30, borderRadius: 8, background: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                    <Icon name="trash" size={14} color="#dc2626" />
                  </motion.button>
                </div>
              </motion.div>
            ))}
          </div>
        )}
      </motion.div>

      {/* Secondary Queue (Skipped / No-Show / Late) */}
      {secondaryQueue.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          style={{
            background: 'white', borderRadius: 20, padding: 24,
            boxShadow: '0 4px 20px rgba(0,0,0,0.05)', marginBottom: 24,
          }}
        >
          <h3 style={{ fontWeight: 700, marginBottom: 16 }}>
            Secondary Queue ({secondaryQueue.length})
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {secondaryQueue.map((p) => (
              <div key={p.id} style={{
                display: 'flex', alignItems: 'center', gap: 12,
                padding: '10px 14px', borderRadius: 12, background: '#f8fafc',
                borderLeft: `3px solid ${p.status === 'no-show' ? '#ef4444' : p.status === 'late' ? '#f59e0b' : '#94a3b8'}`,
              }}>
                <span style={{ fontWeight: 700, fontSize: '0.85rem', color: '#64748b', minWidth: 50 }}>
                  #{p.tokenNumber}
                </span>
                <div style={{ flex: 1 }}>
                  <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>{p.name}</span>
                  <span className={`badge badge-${p.status === 'no-show' ? 'error' : p.status === 'late' ? 'warning' : 'info'}`}
                    style={{ marginLeft: 8 }}>
                    {p.status}
                  </span>
                </div>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                  onClick={() => { reAddFromSecondary(p.id); addToast?.(`${p.name} re-added to queue`, 'success'); }}
                  style={{
                    padding: '6px 14px', borderRadius: 8, fontSize: '0.78rem', fontWeight: 600,
                    background: '#dbeafe', color: '#2563EB', border: 'none', cursor: 'pointer',
                  }}>
                  Re-add
                </motion.button>
              </div>
            ))}
          </div>
        </motion.div>
      )}

      {/* Broadcast */}
      <motion.div
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
        style={{
          background: 'white', borderRadius: 20, padding: 24,
          boxShadow: '0 4px 20px rgba(0,0,0,0.05)',
        }}
      >
        <h3 style={{ fontWeight: 700, marginBottom: 16 }}>
          <Icon name="broadcast" size={20} style={{ verticalAlign: 'middle', marginRight: 8 }} />
          Broadcast Notification
        </h3>
        <div style={{ display: 'flex', gap: 10 }}>
          <input
            value={broadcastMsg}
            onChange={e => setBroadcastMsg(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && sendBroadcast()}
            placeholder="Type a message to all patients..."
            style={{
              flex: 1, padding: '12px 16px', borderRadius: 12,
              border: '1px solid #e2e8f0', fontSize: '0.9rem', outline: 'none',
            }}
          />
          <motion.button
            whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={sendBroadcast}
            style={{
              padding: '12px 24px', borderRadius: 12,
              background: 'linear-gradient(135deg, #2563EB, #14B8A6)',
              color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer',
            }}
          >
            <Icon name="send" size={18} />
          </motion.button>
        </div>
      </motion.div>
    </div>
  );
}

// Need React for useState
import React from 'react';
